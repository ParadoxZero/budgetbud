...
class App extends React.Component<AppProps> {
  ...
  run_pre_run() {
    this.pre_run.forEach((pre_run) =>
      pre_run.condition ? pre_run.action() : null,
    );
  }
  ...
  render(): ReactNode {
    return (
      ...
      >
        <Layout>
          <Layout.Header
            style={{ position: "sticky", padding: 10 }}
            hidden={!this.props.is_header_visible}
          >
            <Header />
          </Layout.Header>
  ...
  render_view() {
    ...
    switch (view) {
      ...
      case View.NoBudgetAvailable:
        return (
          <CreateNewBudgetPage
            is_other_budget_available={this.props.is_budgets_available}
          />
        );
...
function mapStateToProps(state: any): AppProps {
  let budget = null;
  if (
    state.budget.budget_list &&
    state.budget.selected_budget_index !== null &&
    state.budget.budget_list.length > state.budget.selected_budget_index
  ) {
