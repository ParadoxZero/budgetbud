...
export function ShareBudgetModal(props: {
  isOpen: boolean;
  budget_id: string;
  onDone: () => void;
}) {
  ...
  if (props.isOpen && share_key.length == 0) {
    GetShareKey(props.budget_id)
      .then((response) => {
        setShareKey(response.shareKey);
      })
      .catch(() => {
      ...
      <Modal
        ...
        footer={
          <Flex style={{ width: "100%" }} align="center" justify="center">
            <Button type="primary" size="large" onClick={props.onDone}>
              Ok
            </Button>
          </Flex>
        }
      ...
      >
        <Flex align="center" justify="center" vertical gap={10}>
          <Flex gap={10}>
            ...
            />
            <Typography.Text>
              Use the following code to invite others to the budget:{" "}
            </Typography.Text>
          </Flex>
          <Typography.Text copyable style={{ fontSize: 24 }} code>
            {share_key}
          </Typography.Text>
          <Space />
        </Flex>
