...
export class CreateNewBudgetPage extends React.Component<
  ...
  render() {
    return (
      <Flex
        ...
        justify="center"
      >
        <Card title={this.getTitle()}>{this.render_form()}</Card>
      </Flex>
    );
  }
    ...
    switch (this.state.current_step) {
      case WizardStep.CreateBudget:
        return (
          <CreateBudgetForm
            onCancel={
              this.props.is_other_budget_available
                ? () => {
                    store.dispatch(navigate(View.Overview));
                  }
                : undefined
            }
            onFinish={(budget: Budget) => {
              this.setState({
                current_step: WizardStep.AddCategories,
                budget_under_edit: budget,
              });
            }}
          />
        );
      case WizardStep.AddCategories:
        return (
          <AddCategoriesForm
            budget_id={this.state.budget_under_edit!.id}
            onCategoriesAdded={function (): void {
              store.dispatch(navigate(View.Overview));
            }}
